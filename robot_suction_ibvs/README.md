...
4. 运行双位置图像标定，同时生成吸管参考点和预对准偏移：
   ...
   每次微移后脚本会读取机械臂反馈，把 XY 修正回标定开始时的绝对位置，再检查图像中心。
   相对回位指令若仍有残差，脚本默认最多继续执行 3 次反馈补偿（可用
   `--max-return-corrections` 调整），而不是立即失败或直接放宽阈值。矩阵拟合采用每次移动前后
   的机械臂真实反馈位移，不假定指令中的 `1/2 mm` 被完全执行。默认机械臂反馈回位误差上限为
   `0.10 mm`。因为高分辨率画面中 `0.10 mm` 本身就可能对应数个像素，脚本不会再直接要求回位后
   的原始图像坐标完全不变；它会使用最终拟合的矩阵扣除该次真实反馈位移应造成的像素偏移，再以
   `--max-return-error-px`（默认 `3 px`）检查剩余的、机械臂反馈无法解释的图像残差。若该残差超限，
   说明目标、容器、相机发生了位移，或者 HSV 检测切换到了另一个轮廓；此时不应放宽阈值继续标定。
