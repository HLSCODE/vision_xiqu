...
4. 运行双位置图像标定，同时生成吸管参考点和预对准偏移：
   ...
   每次微移后脚本都以与开始时完全相同的 `config.robot.observe_pose` 绝对回位，而不是根据中间的
   位姿反馈再计算一条相对回位指令。所有回位图像
   都与首次观察位中心比较，绝不把某次回位后的中心作为下一轮基准，避免小误差逐轮累积。回位首次
   超限时，脚本默认会再下发一次完全相同的观察位（`--observe-return-attempts 2`）。矩阵拟合仍采用
   每次移动前后的真实机械臂反馈位移，不假定指令中的 `1/2 mm` 被完全执行。回到观察位并完成稳定帧
   采样后，原始图像中心差必须不超过 `--max-return-error-px`（默认 `3 px`）；持续超限时应检查
   `config.robot.observe_pose`、当前工作坐标系、目标是否移动，或 HSV 是否切换到另一轮廓，而不是放宽阈值。
   若示教器手动输入位姿使用的是笛卡尔直线 `MoveL`，可在标定时明确使用相同的回位方式：

   ```bash
   python calibration/calibrate_servo_xy.py --config config.yaml --observe-return-motion linear
   ```

   默认 `joint` 保持原有的关节规划回位；两种方式只影响每次微移后的绝对观察位回程，不改变采样的
   `±1/±2 mm` 直线微移或最终 `servo_A` 的拟合方式。
